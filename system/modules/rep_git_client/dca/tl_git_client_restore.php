 ...
 */
$GLOBALS['TL_DCA']['tl_git_client_restore'] = array
(
	// Config
	'config' => array
	(
		...
		'onload_callback'		=> array(
			array('tl_git_client_restore','onload_callback'),
		),
		'onsubmit_callback'		=> array(
			array('tl_git_client_restore','onsubmit_callback'),
		),
		'disableSubmit'			=> true,
		
		'dcMemory_show_callback' => array(
			array('tl_git_client_restore','showAll')
			),
	...
	(
		'importFiles' => array
		(
			'label'             => &$GLOBALS['TL_LANG']['tl_git_client_restore']['importFiles'],
			'inputType'    		=> 'checkbox',
			'eval'				=> array('multiple'	=> true)
		),  
		'ignoreFiles' => array
		(
			'label'             => &$GLOBALS['TL_LANG']['tl_git_client_restore']['ignoreFiles'],
...
class tl_git_client_restore extends Backend
{
	...
	public function onload_callback(DataContainer $dc)
	{
		...
		$GLOBALS['TL_DCA']['tl_git_client_restore']['fields']['importFiles']['options'] = $arrImportOutput;
	...
	public function onsubmit_callback(DataContainer $dc)
	{
		if ($this->Input->post("submit_startImport"))
		{
			...
			if ((is_array($arrFiles)) && (count($arrFiles)>0))
			{
				...
				$strRequestFilename =sprintf("https://github.com/%s/%s/zipball/%s",$objBranch->repUser,$objBranch->repRepository,$objBranch->repBranch); 
				...
				if (!$objZipFile->hasError())
				{
					...
					foreach ($arrFiles as $filePart)
					{
						...
						$strArchiveFilename = $strZipName.'/'.str_replace($strDestinationPath,"",$file);
